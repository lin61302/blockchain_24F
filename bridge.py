            ...
            for evt in deposits:
                try:
                    ...
                    if receipt.status == 1:
                        ...
                        # Retrieve the wrapped_token address from the contract's mapping
                        wrapped_token_address = dest_contract.functions.wrapped_tokens(token).call()
        ...
        try:
            ...
            for evt in unwraps:
                try:
                    ...
                    from_address = Web3.to_checksum_address(evt.args['from'])
                    to_address = Web3.to_checksum_address(evt.args['to'])
                    amount = evt.args['amount']
                    ...
                    # Build transaction to transfer underlying tokens to 'to_address' on source chain
